...
use Magestore\Webpos\Model\Adapter\DataMapper\AdditionalFieldsProvider;
use Magento\Eav\Api\Data\AttributeOptionInterface;

/**
 * Map product index data to search engine metadata
 *
 * Follow Magento\Elasticsearch\Model\Adapter\BatchDataMapper\ProductDataMapper
 */
class ProductDataMapper implements BatchDataMapperInterface
{
     ...
     * @param AdditionalFieldsProvider $additionalFieldsProvider
    ...
    public function map(array $documentData, $storeId, array $context = [])
    {
        ...
        foreach ($this->getStaticFieldsValue($productIds) as $productId => $fields) {
            $documents[$productId] = array_merge_recursive(
                $documents[$productId],
                $fields
            );
    ...
    /**
     * Get Static Fields Value
     *
     * @param array $productIds
     * @return array
     */
    protected function getStaticFieldsValue($productIds)
    {
        $staticFields = [
            'entity_id',
            'type_id',
        ...
        /** @var \Magestore\Webpos\Model\ResourceModel\Catalog\Product $productResource */
        $productResource = $objectManager->create(
            \Magestore\Webpos\Model\ResourceModel\Catalog\Product::class
        );
        $staticFieldsData = $productResource->getProductStaticFieldsInformation(
            $staticFields,
            $productIds
        );
        $staticFieldsValue = [];
        foreach ($staticFieldsData as $data) {
            $staticFieldsValue[$data['entity_id']] = [
