...
namespace Magestore\Rewardpoints\Model\Plugin\SalesRule\Quote;
/**
 * Class Discount
...
class Discount
{
    ...
    )
    {
        ...
        if (is_array($result)) {
            if ($total->getRewardpointsDiscount() && $result['value'] != 0) {
                $result['value'] = min(0, $result['value'] + $total->getRewardpointsDiscount());
            }
        }
        return $result;
    }
