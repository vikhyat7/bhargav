...
define([
    ...
    'Magestore_FulfilSuccess/js/model/repository/packRequestItem'
], function ($, ko, __, PackRequestItem, PackRequestItemRepository) {
    'use strict';

    return {
        EXTERNAL_SELECTOR: {
            INCREASE_BUTTON: ".col-packed-qty .os-increase-button"
        },
        ROLE: {
        ...
        scan: function (el) {
            ...
            if (source && source.length > 0) {
                var item = ko.utils.arrayFirst(source, function (item) {
                    return (PackRequestItemRepository.isValidItem(item, PackRequestItem.ITEM_BARCODE, value));
                });
                if (item) {
                    var itemId = item[PackRequestItem.PACK_REQUEST_ITEM_ID];
                    self.packItem(itemId);
                } else {
         ...
         * @param itemId
         */
        packItem: function (itemId) {
            var self = this;
            var increaseBtns = $(self.EXTERNAL_SELECTOR.INCREASE_BUTTON + '[data-target="os_packed_items_' + itemId + '"]');
