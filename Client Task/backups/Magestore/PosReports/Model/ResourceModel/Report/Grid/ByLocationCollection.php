...
/**
 * Class ByLocationCollection
 *
 * Used to create By Location Collection
 */
class ByLocationCollection extends AbstractCollection
{
    ...
    public function getAggregatedColumns()
    {
        ...
        $periodTypeExpr = new \Zend_Db_Expr(
            sprintf(
                "'%s'",
                $periodType
            )
        );
        $periodFormat = $this->getPeriodFormat($periodType);
        $aggregatedColumns = [
            'period' => $periodFormat,
            'period_type' => $periodTypeExpr,
        ...
        if ($periodType == PeriodType::WEEK) {
            unset($aggregatedColumns['period']);
            $aggregatedColumns['yearweek'] = "yearweek";
            $this->setGroupBy("yearweek");
        } else {
            $this->setGroupBy($periodFormat);
        }
