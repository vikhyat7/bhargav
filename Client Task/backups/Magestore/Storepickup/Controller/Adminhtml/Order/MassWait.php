 ...
 */
class MassWait extends \Magestore\Storepickup\Controller\Adminhtml\Order
{
    /**
     * Execute action.
     *
...
//        $collection = $this->_massActionFilter->getCollection($this->_createMainCollection());
        ...
        foreach ($collection as $item) {
            if($item->getStorepickupStatus()<StorepickupStatus::STOREPICUP_WAIT_FOR_PICKUP) {
                $item->setStorepickupStatus(StorepickupStatus::STOREPICUP_WAIT_FOR_PICKUP);
                $item->save();
            }
            $storeId = $item->getStorepickupId();
        }

        $this->messageManager->addSuccess(__('A total of %1 record(s) have been Wait for Pickup.', $collection->getSize()));
