 ...
 */
class MassReceived extends \Magestore\Storepickup\Controller\Adminhtml\Order
{
    /**
     * Execute action.
     *
...
//        $collection = $this->_massActionFilter->getCollection($this->_createMainCollection());
        ...
        foreach ($collection as $item) {
            if($item->getStorepickupStatus()<StorepickupStatus::STOREPICUP_RECEIVED) {
                $item->setStorepickupStatus(StorepickupStatus::STOREPICUP_RECEIVED);
                $item->save();
            }
            $storeId = $item->getStorepickupId();
        }

        $this->messageManager->addSuccess(__('A total of %1 record(s) have been Received.', $collection->getSize()));
