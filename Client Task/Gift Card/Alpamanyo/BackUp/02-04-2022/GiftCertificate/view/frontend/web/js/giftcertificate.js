...
require([
    ...
    jQuery('.gift-content .giftemplates img.template-image:first').addClass('active');
		
		jQuery(document).on('click', ".template-image", function () {

			jQuery(".giftemplates  img").removeClass('active');
    	jQuery(this).addClass('active');
		 		...
		 		jQuery(".img-fluid.lazyOwl").attr('src',img);
		 		jQuery(".img-fluid").attr('src',img);		 		
		 		
		 		jQuery("#template_id").val(jQuery(this).siblings('.temp_id').val())
		 		
		});

		jQuery(document).on('click', "img.img-fluid", function() {
			var img=jQuery(this).attr('src');
		 	// console.log(img);
			// alert("call123");
			jQuery(".mfp-img").attr('src',img);
		});
			...
			return false;
 			...
 			jQuery(document).on('click','.switch-prices', function(){
 				...
 				// alert(jQuery(this).text());
				  var val = $.mage.__('Buy With Your Price');
				  if(jQuery(this).text()==  val || jQuery(this).text()== ('Set the price yourself') || jQuery(this).text()== ('Buy With Your Price'))
				  {
				  	...
				  	$('#manual-price').keyup(
						function() {
				  		...
				  		jQuery(this).text($.mage.__('Buy With System Price'));
				  ...
				  else
				  {
						...
						jQuery(".gift-row span").css("display","none");
						jQuery("#gift-template-label span").css("display","none");
						jQuery(this).text($.mage.__('Buy With Your Price'));
						jQuery("select.gift-prices").toggle();
				    	jQuery("input.manual-price").toggle();
