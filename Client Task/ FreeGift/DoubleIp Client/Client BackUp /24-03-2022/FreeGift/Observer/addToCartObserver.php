		...
		if($isActive) {
			...
			// For Allow Multi Free Gift Start
        		...
        		if($this->_freeGiftHelper->getAllowMultiples() == 1){
        			...
        			$writer = new \Zend\Log\Writer\Stream(BP . '/var/log/addToCartObserver.log');
					...
					$remove_pro_sku1=array();
					if($items) {
						foreach($items as $item) {
							if ($item->getPrice() != '0' && $item->getQty() > 1) {

									$remove_pro_sku1 = $item->getQty()."<br>";	
									$logger->info('=====old======');
									$logger->info($remove_pro_sku1);
								}
							if($item->getPrice() == '0' || $item->getPrice() == '0.0000'){
								$logger->info('======call in setQty=====');
								// $logger->info($MultipleQty));
								// $item->setQty($remove_pro_sku1);
								$item->setQty($remove_pro_sku1)->save();
			...
			// For Allow Multi Free Gift End
				...
				if($this->_freeGiftHelper->getAllowMultiples() == 1)
				{
					$FreeGiftQty = (int)$remove_pro_sku1; 
					// $FreeGiftQty = 10; 
					$logger->info('=====in condition=========');
					$logger->info($FreeGiftQty);
				}else{
