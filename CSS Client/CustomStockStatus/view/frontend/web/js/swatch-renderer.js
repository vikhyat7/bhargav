...
define([
    ...
    setTimeout(function(){
        ...
        var urlperameter = url.split('#');
        var perameter = urlperameter[1].split('=');
        var color_id = perameter[0];
        var attr_id = perameter[1];
        var myid = '#option-label-color-'+color_id+'-item-'+attr_id;
        $(myid).trigger("click");
    }, 1000);
    // Custom End
    ...
    $.widget('mage.SwatchRenderer', {
         ...
         * @private
         */
        _isSimpleConfigurableEnable: function ($widget) {
            if (typeof $widget.options.isSimpleConfigurable !== 'undefined' && ($widget.options.isSimpleConfigurable == '1' || $widget.options.isSimpleConfigurable == 1)) {
                return true;
            }else{
                return false;
            }
        ...
        _OnClick: function ($this, $widget) {
            ...
            $widget.options.jsonConfig.customlable[this.getProduct()];
            ...
            var option1, 
                option2 = undefined;
            if ($parent.parent().parent().find('.custom_stock_status').hasClass('custom_stock_status')) {
                $parent.parent().parent().find('.custom_stock_status').empty();
            }
            if ($this.hasClass('selected')) {
                ...
                option1 = this.getProduct();
                if (option1 != undefined) {
                    option2 = this.getProduct();
                }
            ...
            // if ($widget.options.jsonConfig.changeConfigProduct == '1' && $parent.parent().parent().find('.custom_stock_status').hasClass('custom_stock_status')) {
               // if (option2 != undefined) {
                    //var content = $widget.options.jsonConfig.customlable[option2];
                 //   if (content != undefined) {
                      //  var span = '<img src="'+content.icon+'" id="custome_stock_icon" class="custom_stock_status_icon" alt="" title=""><span class="customstockstatus status_'+attributeId+'">'+content.label+'</span>';
                        //$parent.parent().parent().find('.custom_stock_status').html(span);
                        //$('.product-info-stock-sku').html(span);
                 //   } else {
                 //       $parent.parent().parent().find('.custom_stock_status').empty();
                 //   }
               // }
            ...
            if ($widget.options.jsonConfig.changeConfigProduct == '1' && $parent.parent().parent().find('.custom_stock_status').hasClass('custom_stock_status')) {
                if (option2 != undefined) {
                    var content = $widget.options.jsonConfig.customlable[option2];
                    if (content != undefined) {
                        var span = '<img src="'+content.icon+'" id="custome_stock_icon" class="custom_stock_status_icon" alt="" title=""><br><span class="customstockstatus status_'+attributeId+'">'+content.label+'</span>';
                        $parent.parent().parent().find('.custom_stock_status').html(span);
                    } else {
                        $parent.parent().parent().find('.custom_stock_status').empty();
                    }
            ...
            var isSCEnable = $widget._isSimpleConfigurableEnable($widget);
            if(isSCEnable){
