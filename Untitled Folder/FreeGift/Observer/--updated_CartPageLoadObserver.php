...
{ 
	...
	public function addProductToCart($skus,$qty)
	{
		...
		// 	$new = $this->_productRepository->getById($item->getProductId(), false, $storeId, true);

			
		// 	if($new->getTypeId() == 'configurable') {
		// 		// echo "string";exit();
		// 		$oldsku = $item->getSku();
		// 	}
		...
		// 	// echo "<br />";
		// }
		// $new = $this->_productRepository->get($oldsku);
		// $oldsku_Id = $new->getId();
		// echo $oldsku_Id;
		// echo $oldsku;
		// exit();
		...
		if($isActive) {
			...
			foreach($freeGiftSkus as $sku)
			{
				...
				// var_dump($sku);
				// var_dump($freeGiftProduct->getId());exit();
				$additionalOptions = [];
				...
				if($freeGiftProduct->getTypeId() == 'configurable'){
					// echo "Called";exit();
					// if ($freeGiftProduct->getId()) {
					// 	$loadProduct = $this->_productRepository->getById($freeGiftProduct->getId(), false, $storeId, true);		
					// }else{

						$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
						$configProduct = $objectManager->create('Magento\Catalog\Model\Product')->load($freeGiftProduct->getId());
						// $configProduct = $objectManager->create('Magento\Catalog\Model\Product')->load($oldsku_Id);
